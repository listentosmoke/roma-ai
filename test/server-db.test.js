...
test('migrations apply from an empty database', () => {
  ...
  assert.equal(currentSchemaVersion(db), '0008_memory_embeddings');
  const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all().map((r) => r.name);
  for (const expected of ['memories', 'people', 'relationships', 'identity_evidence', 'sessions', 'audit_events', 'consent_records', 'voice_profile_refs', 'voice_templates', 'face_templates', 'tombstones', 'operation_ids', 'schema_migrations', 'memory_embeddings']) {
